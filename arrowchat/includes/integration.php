	...
	*/
	...
	function get_user_id() 
	{
		...
		if (!empty($_COOKIE['ci_session'])) {
			...
			$cisess_connect = mysql_connect(DB_SERVER, DB_USERNAME, DB_PASSWORD);
			if (!$cisess_connect) {
			 die('<div class="error">' . mysql_error() . '</div>');
			}
			$cisess_query = "SELECT data FROM ci_sessions WHERE id = '$cisess_cookie' LIMIT 1";
			
			mysql_select_db(DB_NAME, $cisess_connect);
			$cisess_result = mysql_query($cisess_query, $cisess_connect);
			if (!$cisess_result) {
			 die("Invalid Query");
			}
			$cisess_row = mysql_fetch_assoc($cisess_result);
			session_decode($cisess_row['data']);	
